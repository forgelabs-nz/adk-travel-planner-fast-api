...
# Setup basic logging (Cloud Logging will be configured by the server if needed)
...
async def save_pitch_artifact(
    tool_context: ToolContext,
    filename: str,
    content: str
) -> dict[str, str]:
    """Save a movie pitch as an artifact in the session.
    ...
    Returns:
        dict[str, str]: {"status": "success", "filename": filename, "version": version}
    """
    try:
        ...
        # Create a Part with inline_data (Blob)
        # Include display_name for better UI rendering
        ...
        # Save the artifact and await the result
        version = await tool_context.save_artifact(filename, artifact_part)

        logging.info(f"Saved artifact '{filename}' v{version}")
        return {"status": "success", "filename": filename, "version": version}

    except Exception as e:
        logging.error(f"Error saving artifact '{filename}': {e}")
        import traceback
...
file_writer = Agent(
    ...
    - Use your 'save_pitch_artifact' tool to save the pitch as an artifact with the following arguments:
        - for a filename, use the movie title
        - For the 'content', include:
            - The PLOT_OUTLINE
